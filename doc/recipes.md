...
function _G.get_oil_winbar()
  ...
  -- Set up highlight group for modified indicator (amber 500)
  vim.api.nvim_set_hl(0, "OilWinbarModified", { fg = "#f59e0b" })
  
  local path_text
  if dir then
    path_text = vim.fn.fnamemodify(dir, ":~")
  else
    -- If there is no current directory (e.g. over ssh), just show the buffer name
    path_text = vim.api.nvim_buf_get_name(0)
  end
  
  -- Check if buffer is modified and add indicator
  if vim.bo[bufnr].modified then
    return path_text .. " %#OilWinbarModified#[modified]%*"
  else
    return path_text
  end
end
